...
public class app {
        ...
        public StartMenu() {
            ...
            if (username == null || username.isEmpty()) {
                ...
                loginButton.addActionListener(e -> {
                    ...
                    // After login, refresh the menu to show username
                    showStartMenu();
                });
                add(loginButton, gbc);
            } else {
                // Show username label if logged in
    ...
    static class GamePanel extends JPanel implements KeyListener {
        ...
        private Image truckImageRight;
        private Image truckImageLeft;
        private Timer timer;
        ...
        private final int NUM_TRUCKS = 6; // Number of trucks to spawn
        ...
        public GamePanel() {
            // Load images
            frogImage = new ImageIcon(getClass().getResource("/img/frog.png")).getImage();
            truckImageRight = new ImageIcon(getClass().getResource("/img/icecreamtruck.png")).getImage();
            truckImageLeft = app.createFlipped(truckImageRight);
            logImage = new ImageIcon(getClass().getResource("/img/log.png")).getImage(); // Add log image loading
        ...
        private void playBackgroundMusic() {
            try {
                // For sounds, use getResourceAsStream
                InputStream audioSrc = getClass().getResourceAsStream("/sounds/FroggysMelody.wav");
        ...
        private static void submitGameScore(int score) {
            try {
                ...
                URL url = new URL("https://frogger.mcbehub.de/api/v0/user/addgame");
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (!gameOver) {
                for (Truck truck : trucks) {
                    Image img = truck.movingRight ? truckImageRight : truckImageLeft;
                    g.drawImage(img, truck.getX(), truck.getY(), GRID_SIZE, GRID_SIZE, this);
                }
            }
    ...
    private static class LoginWindow extends JDialog {
        ...
        private void attemptLogin() {
            try {
                URL url = new URL("https://frogger.mcbehub.de/api/v0/user/login");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                conn.setRequestProperty("Content-Type", "application/json");
                conn.setDoOutput(true);
    ...
    public static void main(String[] args) {
        ...
        // For images
        ImageIcon icon = new ImageIcon(app.class.getResource("/img/icon.png"));
        frame.setIconImage(icon.getImage());
        frame.setVisible(true);
    }

    private static Image createFlipped(Image original) {
        // Create a new image with the same dimensions and type as the original
        BufferedImage flippedImage = new BufferedImage(original.getWidth(null), original.getHeight(null), BufferedImage.TYPE_INT_ARGB);

        // Get the graphics context of the new image
        Graphics2D g2d = flippedImage.createGraphics();

        // Flip the image horizontally
        g2d.drawImage(original, original.getWidth(null), 0, -original.getWidth(null), original.getHeight(null), null);
